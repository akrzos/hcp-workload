#!/usr/bin/env bash
# Ramp count of Secrets on a cluster
# 40,000 Secrets (Each with 8 keys of 1KiB) * 10 Namespaces will trigger KAS OOM on m5.xlarge serving nodes (500 QPS) (With consecutive tests)
# 50,000 Secrets (Each with 8 keys of 1KiB) * 10 Namespaces will trigger KAS OOM on m5.xlarge serving nodes (100 QPS) (With consecutive tests)
# 50,000 Secrets test above will succeed when just test singular (at least one time it has succeeded) (Suggesting both rate and sequence of tests play a role in triggering KAS OOM)
# set -e
set -o pipefail
...
export CONFIGMAPS=0
export CM_KEY_COUNT=0
export CM_VALUE_SIZE=1024
# export SECRETS=0 # (Ramped in a variable below)
export SECRET_KEY_COUNT=8
...
# Range of secrets counts
# secret_counts+=("10000" "20000" "30000" "40000" "50000" "60000" "70000" "80000" "90000" "100000")
secret_counts+=("40000" "50000" "60000" "70000" "80000" "90000" "100000")

test_dir="results/${ts}-secret-count"
run_log_file="${test_dir}/run.log"
for i in "${!secret_counts[@]}"; do
  export SECRETS=${secret_counts[$i]}
  export METRICS_DIRECTORY="${test_dir}/${i}-${SECRETS}"
  ...
  echo "$(date -u +%Y%m%d-%H%M%S) :: Running Test: $i, SECRETS: ${SECRETS}" | tee -a "${run_log_file}"
