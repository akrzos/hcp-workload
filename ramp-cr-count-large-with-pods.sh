...
test_dir="results/${ts}-ramp-large-cr-counts-with-pods"
...
for i in "${!cr_counts[@]}"; do
  ...
  time kube-burner-ocp --local-indexing --qps ${QPS} --burst ${BURST} --timeout ${timeout} --enable-file-logging=False init -c hcp-workload/job-workload.yml 2>&1 | tee ${kb_log_file}
  # time kube-burner-ocp --local-indexing --qps ${QPS} --burst ${BURST} --timeout ${timeout} --enable-file-logging=False init -c hcp-workload/job-workload.yml --log-level debug 2>&1 | tee ${kb_log_file}
  kb_rc=$?
